//! Phase 8f: cross-SDK interop between turbomcp4 and the official Rust MCP SDK
//! (`rmcp`), both directions, in-process over a duplex pipe.
//!
//! rmcp's latest protocol version is `2025-11-25` — turbomcp4's legacy path — so
...
async fn turbomcp4_client_drives_rmcp_server() {
    ...
    // turbomcp4 client on the other end (legacy = rmcp's protocol version).
    let (c_rd, c_wr) = split(client_io);
    let transport = LineTransport::new(BufReader::new(c_rd), c_wr, SerdeJsonCodec);
    let client = ClientBuilder::new("turbomcp4-client", "1.0.0")
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn rmcp_client_drives_turbomcp4_server() {
    let (server_io, client_io) = tokio::io::duplex(64 * 1024);

    // turbomcp4 dual-stack server on one end (rmcp speaks the legacy path).
    let (s_rd, s_wr) = split(server_io);
    ...
    let result = client
        ...
        .await
        .expect("rmcp call_tool against turbomcp4");
