...
//! round-trip through turbomcp4 vs the official Rust SDK (`rmcp`), each SDK
//! driving **both ends** (its own client and server) over an in-process
//! `tokio::io::duplex` pipe.
//!
//! Both sit on the `2025-11-25` protocol version (rmcp's latest, turbomcp4's
...
//!   `cd crates/turbomcp4-interop && cargo bench --bench sdk_comparison`
...
use turbomcp4::client::{Client, ClientBuilder, ConnectMode};
use turbomcp4::prelude::*;
use turbomcp4::{LegacySessionAdapter, SerdeJsonCodec, serve};
...
// ---- a turbomcp4 server with one `add` tool --------------------------------
...
/// turbomcp4 client ↔ turbomcp4 server, connected and handshaken.
async fn connect_turbomcp4() -> Client {
...
fn bench_call_tool(c: &mut Criterion) {
    ...
    let turbo = rt.block_on(connect_turbomcp4());
    c.bench_function("turbomcp4/call_tool_roundtrip", |b| {
