...
use turbomcp4_transport_stdio::{LineTransport, StdioError};
...
async fn server_handles_discover_list_and_call_over_stdio_framing() {
    ...
    let transport = LineTransport::new(
        ...
        server_tx,
        turbomcp4_codec::SerdeJsonCodec,
    );

    let server_task = tokio::spawn(turbomcp4_service::serve(transport, dispatcher));
