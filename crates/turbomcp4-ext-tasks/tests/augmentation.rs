...
use turbomcp4_core::{Implementation, JsonRpcMessage, JsonRpcRequest, McpError, McpResult};
use turbomcp4_ext_tasks::{EXTENSION_ID, TasksExtension};
...
async fn non_declaring_client_never_gets_a_task() {
    ...
    let meta = json!({ "io.modelcontextprotocol/protocolVersion": "DRAFT-2026-v1" });
    let out = call(
        &mut svc,
        JsonRpcRequest::new(
            1,
