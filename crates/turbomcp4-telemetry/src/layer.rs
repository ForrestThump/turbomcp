...
use turbomcp4_core::{JsonRpcMessage, RedactedSubject, meta};
...
mod tests {
    ...
    use turbomcp4_core::{JsonRpcRequest, JsonRpcResponse};
    ...
    fn composes_as_mcp_service() {
        ...
        fn assert_mcp_service<S: turbomcp4_service::McpService>(_: &S) {}
        #[derive(Clone)]
        struct Dispatcher;
        impl Service<JsonRpcMessage> for Dispatcher {
            type Response = Option<JsonRpcMessage>;
            type Error = turbomcp4_service::ProtocolError;
            type Future = std::future::Ready<Result<Self::Response, Self::Error>>;
            fn poll_ready(&mut self, _: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
                Poll::Ready(Ok(()))
            }
