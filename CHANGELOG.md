...
### Phase 4 — LOW audit follow-up (2026-04-26)

A second sweep through the LOW backlog covering API-shape changes that the
first pass deferred. Most are additive (new fields with backwards-compatible
defaults, new constructors) but a few rename or remove unused public surface.

- **`turbomcp-protocol::jsonrpc::JsonRpcResponsePayload` enforces JSON-RPC §5 mutual exclusion** — `crates/turbomcp-protocol/src/jsonrpc.rs`. Replaced the `#[serde(untagged)]` derive (which silently picked `Success` on `{ "result": ..., "error": ... }`, dropping the error, and produced a misleading "missing field `result`" diagnostic on `{}`) with a custom `Deserialize` impl that requires *exactly one* of `result` / `error`. Both ambiguous shapes now fail with a clear single-line message: "JSON-RPC response must contain exactly one of `result` or `error`[, not both]". `Serialize` still uses the untagged shape so the wire format is unchanged.
- **`turbomcp-protocol::error_utils` deleted** — the module was a `Result<T, String>` shim from the v1 era with zero non-self consumers; its presence was actively misleading because every modern call site uses the unified `McpError`. Removed the file and the `pub mod error_utils` from `lib.rs`.
- **`turbomcp-protocol::types::CompletionData::validate()` added** — `crates/turbomcp-protocol/src/types/completion.rs`. The existing rustdoc claimed "Must not exceed 100 items" with no enforcement. New `validate()` method returns `Err` when `values.len() > MAX_COMPLETION_VALUES` (100 per MCP 2025-11-25), and a public constant exposes the cap so callers can size buffers correctly.
- **`turbomcp-protocol::context::rich`: stronger doc warning on `SESSION_STATE`** — `crates/turbomcp-protocol/src/context/rich.rs`. The unbounded `DashMap` was already documented as needing manual cleanup; the warning is now upgraded to call out OOM risk explicitly and direct multi-tenant deployments to `SessionStateGuard`.
- **`turbomcp-protocol::types::PingParams.data` documented as non-spec** — `crates/turbomcp-protocol/src/types/ping.rs`. The field is a TurboMCP echo-payload extension used by the WebSocket reconnect probe; it never appeared in MCP `schema.ts:578-581`. Added prominent rustdoc so callers writing portable code know to leave it `None` (extra fields are tolerated by spec-strict peers but not echoed back).
- **`turbomcp-transport::bidirectional::MessageRouter::set_default_handler` exposed** — `crates/turbomcp-transport/src/bidirectional.rs`. The `default_handler` field already existed but had no setter, so it was permanently `None`. Added `set_default_handler` / `clear_default_handler` so callers can install a fallback for unrouted messages.
- **`turbomcp-transport::child_process` stderr task tracked** — `crates/turbomcp-transport/src/child_process.rs`. Pre-3.2.0 the stderr drain task was bound to `_stderr_task` and dropped immediately, so `stop_process` couldn't abort it on shutdown — it relied on stderr-EOF after `kill_on_drop` to make the task exit on its own. New `_stderr_task` field on the transport and explicit `.abort()` calls in `stop_process` for stdin/stdout/stderr drain tasks.
- **`turbomcp-transport::resilience::HealthStatus` gains `Recovering` / `Degrading` variants** — `crates/turbomcp-transport/src/resilience/health.rs`. Pre-3.2.0 below-threshold success and below-threshold failure both reported `Unknown`, hiding the direction of motion from dashboards. New variants distinguish "starting to recover" from "starting to fail" while preserving `Unknown` for the no-checks-yet state. Existing tests updated.
- **`turbomcp-transport::resilience::TurboTransport::transport_type/endpoint` are now lock-free** — `crates/turbomcp-transport/src/resilience/transport.rs`. Pre-3.2.0 both fell back to `try_lock()` and on contention returned `TransportType::Stdio` (a fabricated, possibly-wrong value) and `None` respectively. The values are immutable for the wrapper's lifetime, so they are now snapshotted at construction and served from cached fields without any lock acquisition.
- **`turbomcp-transport::security::SessionSecurityManager::validate_session` holds the lock across rotation** — `crates/turbomcp-transport/src/security/session.rs`. Pre-3.2.0 the rotation path dropped the lock between read and write, so a concurrent `validate_session` observing the *old* id could succeed mid-rotation. `parking_lot::Mutex` has no re-entrancy issue; the entire rotation now runs as one continuous critical section.
- **`turbomcp-transport::security::OriginConfig::production` constructor + `is_dev_permissive` predicate** — `crates/turbomcp-transport/src/security/origin.rs`. The default `OriginConfig` is dev-permissive (no allowlist, localhost allowed). New `production(allowed_origins)` constructor builds the safe shape; new `is_dev_permissive()` predicate lets callers refuse to bind to a non-loopback address (or at least log a `WARN`) when their config is the dev default.
- **`turbomcp-transport::security::AuthConfig` zeroes API keys on Drop** — `crates/turbomcp-transport/src/security/auth.rs`. New `Drop` impl drains the `HashSet<String>` and calls `zeroize::Zeroize::zeroize` on each key before the allocation is released, so a process core dump or memory scanner cannot recover keys that have left scope. New workspace-pinned `zeroize` dep on `turbomcp-transport`. Test sites that previously used struct-update syntax (`..Default::default()`) updated to field-by-field construction (E0509: cannot move out of `Drop`-impl'd value).
- **`turbomcp-server::ConnectionCounter::try_acquire_arc` CAS loop unbounded** — `crates/turbomcp-server/src/config.rs`. Pre-3.2.0 the loop was capped at 1000 iterations with a `tracing::error!` "possible contention bug" log on overflow. Under genuine `accept()` storms 1000 is easily exceeded and the cap caused false-alarm errors; unbounded CAS guarantees forward progress because each iteration either advances `current` or witnesses another thread's advance.
- **`turbomcp-server::CompositeHandler::match_prefix` is alloc-free** — `crates/turbomcp-server/src/composite.rs`. Replaced the per-iteration `format!("{prefix}{sep}")` allocation with direct byte comparison; the iteration is still O(n) over handlers but now zero-allocation per call.
- **`turbomcp-core::router::parse_request_from_value` added; line transports parse incoming JSON once** — `crates/turbomcp-core/src/router.rs`, `crates/turbomcp-server/src/transport/line.rs`. Pre-3.2.0 STDIO/TCP/Unix paths called `serde_json::from_str` once into a `Value` (to detect server-to-client responses), then `router::parse_request` re-ran the JSON parser over the same string. The new helper accepts a pre-parsed `Value`, saving a full parse on the hottest server path.
- **`turbomcp-client::Client::connect_tcp` rustdoc documents DNS limitation** — `crates/turbomcp-client/src/client/core.rs`. Pre-3.2.0 the example showed `"localhost:8765"` but the implementation parsed via `SocketAddr::from_str` which rejects DNS hostnames. Doc now states explicitly that callers must provide a numeric `SocketAddr` (use `tokio::net::lookup_host` to resolve first).
- **`turbomcp-client::SessionManager::start_health_monitoring` no longer holds the write lock during pings** — `crates/turbomcp-client/src/client/manager.rs`. Pre-3.2.0 the write lock was held for `timeout × N` (default 5 s × N) per pass, blocking every other read of the session map. New impl snapshots `(id, client Arc)` pairs under a read lock, releases the lock, pings each in parallel via `JoinSet`, and reacquires the write lock just long enough to apply state transitions for sessions still in the map.
- **`turbomcp-client::sampling::ServerInfo` renamed to `LlmServerInfo`** — `crates/turbomcp-client/src/sampling.rs`. Pre-3.2.0 the prelude re-exported `ServerInfo` from the LLM sampling module under the same name as the MCP `Implementation`/`InitializeResult.server_info` shape, causing prelude-level shadowing. Now the canonical name is `LlmServerInfo`; `ServerInfo` is kept as a deprecated alias for one release. Get-server-info trait method updated to the new name.
- **`turbomcp-client::Client` experimental Tasks methods gate on `initialized`** — `crates/turbomcp-client/src/client/core.rs`. `get_task` / `cancel_task` / `list_tasks` / `get_task_result` now return `invalid_request("Client not initialized")` if called before `initialize()`, matching every other operation. Per SEP-1686 tasks are scoped to an initialized session.
- **`turbomcp-client::Client::shutdown` and request handler logs stripped of emoji** — `crates/turbomcp-client/src/client/core.rs`. Removed the 🛑 / ✅ / ❌ / 📤 / 🔄 / 📋 / ⚠️ prefixes from `tracing` lines so structured-log destinations get clean message strings.
- **`turbomcp-auth::DcrClient` redacts `initial_access_token` in `Debug`** — `crates/turbomcp-auth/src/oauth2/dcr.rs`. Pre-3.2.0 the derived `Debug` printed the IAT verbatim; the manual impl now substitutes `[REDACTED]` for the `Some(_)` form and `<none>` for `None`, matching the pattern already used by `TokenInfo`/`AccessToken`/`AuthCredentials`.
- **`turbomcp-auth::JwtValidator` logs SHA-256 prefix instead of raw subject claim** — `crates/turbomcp-auth/src/jwt/validator.rs`. The successful-validation `debug!` line previously logged the raw `sub` claim — a per-user identifier and PII in many deployments. Now logs the first 4 bytes of `Sha256(sub)` as `sha256:xxxxxxxx`, preserving correlation across log lines for the same user without storing the user-id in cleartext.
- **`turbomcp-dpop`: `DEFAULT_CLOCK_SKEW_SECONDS = 60` (was 300)** — `crates/turbomcp-dpop/src/{lib,proof}.rs`. Pre-3.2.0 `DpopProofGenerator::with_nonce_tracker` used `MAX_CLOCK_SKEW_SECONDS = 300` as its default tolerance; combined with a 60s proof lifetime the effective acceptance window was 6 minutes. Now defaults to 60 s, matching the proof lifetime, for an effective 2-minute acceptance window per RFC 9449 §11.1's recommendation. The 5-minute upper bound is preserved as the explicit cap; operators that need a wider window can set it.
- **`turbomcp-macros::ToolAttrs::parse` rejects unknown keys** — `crates/turbomcp-macros/src/tool.rs`. Pre-3.2.0 a typo like `descriptio = "..."` was silently dropped (`let _ = meta.value();`), leaving the resulting tool with the default description and no diagnostic. Now an unknown key produces a `syn::Error` with the list of expected keys at compile time.
- **`turbomcp-macros::#[server(transports = "stdio")]` shows the deprecation message first** — `crates/turbomcp-macros/src/server.rs`. Pre-3.2.0 the deprecation `Err` only fired after a successful `syn::ExprArray` parse, so users writing the v2 string-form (`transports = "stdio"`) hit a generic `expected '['` diagnostic before the migration guidance. Reordered so the deprecation error is unconditional for the `transports` ident.
- **`turbomcp-wasm-macros::ComponentAttrs::parse` returns `syn::Result`** — `crates/turbomcp-wasm-macros/src/server.rs`. Unknown keys (typo'd `descriptio`, etc.) now produce a `syn::Error` instead of silently defaulting to `description = "Tool"`. Non-string-literal values for `description`/`version` also error. `parse_tags_array` rewritten as a token-tree walker that recurses into nested groups so `tags` matches must be a bare ident inside an attribute, not a substring inside an unrelated description.
- **`turbomcp-wasm::wasi::transport::JsonRpcResponse::is_valid_version`** — `crates/turbomcp-wasm/src/wasi/transport.rs`. Pre-3.2.0 the `jsonrpc` field was `pub(crate)` and `#[allow(dead_code)]`, so a server returning `"jsonrpc": "1.0"` (or omitting it) round-tripped without complaint. New helper enforces the spec-required `"2.0"` for callers that want strict validation.
- **`turbomcp-wasm::auth::jwt::JwtConfig::leeway_seconds` semantics documented inline** — `crates/turbomcp-wasm/src/auth/jwt.rs`. The asymmetric `now > exp + leeway` (`exp` check) vs `now + leeway < nbf` (`nbf` check) form is correct (both extend the validity window outwards by `leeway_seconds`) but at-a-glance reads as a typo. Inline comments now explain the symmetry.
- **`turbomcp-openapi::lookup_ref` follows reference chains up to 10 levels with cycle detection** — `crates/turbomcp-openapi/src/provider.rs`. Pre-3.2.0 only one level of indirection was followed, so `Foo → Bar → Baz` chains broke at the second hop. New loop with `MAX_DEPTH = 10` and a `HashSet` of visited names prevents cycles.
- **`turbomcp-cli::dev::is_command_available` uses the `which` crate (cross-platform)** — `crates/turbomcp-cli/src/dev.rs`. Pre-3.2.0 it shelled out to the Unix-only `which` binary; on Windows the call always returned `false`. New implementation calls `which::which(cmd).is_ok()`, working uniformly on Windows / macOS / Linux. New `which = "8.0"` dep.
- **`turbomcp-proxy::config`: dead `ProxyConfig` and `IdMappingStrategy` removed** — `crates/turbomcp-proxy/src/config.rs`. Both had zero non-self consumers in the workspace and misled API readers into thinking the proxy honored `session_timeout`/`max_sessions`/`request_timeout` knobs.
- **`turbomcp-proxy::runtime::is_localhost` accepts both bracketed and unbracketed IPv6** — `crates/turbomcp-proxy/src/runtime/mod.rs`. Pre-3.2.0 the `matches!` literal listed both `"::1"` and `"[::1]"` separately; callers obtaining the host string from `Url::host_str()` (which strips brackets) and from raw config text both worked, but the duplication invited drift. New impl normalizes by stripping `[` / `]` once before comparison.
- **`turbomcp-proxy::BackendTransport::Http.endpoint_path` field added; `--backend-path` CLI flag** — `crates/turbomcp-proxy/src/{config,proxy/backend,cli/args}.rs`. Pre-3.2.0 `endpoint_path: "/mcp".to_string()` was hardcoded in `BackendConnector::new`, so servers mounting MCP at `/api/mcp` could not be proxied. New `Option<String>` field on both `BackendConfig::Http` and `BackendTransport::Http` (defaults to `/mcp` when `None`); new `--backend-path` flag on `BackendArgs` (requires `--http`); new `RuntimeProxyBuilder::with_http_backend_path(url, path, auth)` helper.
- **`turbomcp-proxy::ProxyError::Backend` carries the upstream JSON-RPC code; `RestAdapter` and `ProxyService` propagate it** — `crates/turbomcp-proxy/src/{error,proxy/{backend,service},adapters/rest}.rs`. Pre-3.2.0 every backend failure flattened to `-32603 Internal error` because `ProxyError::backend(format!("…"))` stringified the underlying `McpError`. New `upstream_jsonrpc_code: Option<i32>` field, `ProxyError::backend_with_code(msg, code)` constructor, and `ProxyError::upstream_jsonrpc_code()` accessor. Each `BackendConnector` method (`call_tool`, `list_tools`, `read_resource`, etc.) now captures `e.jsonrpc_error_code()` from the underlying client error and threads it through. The `ProxyError → McpError` conversion preserves the code via `ErrorKind::from_i32`. The REST adapter's tool-call error handlers replace their hardcoded `-32603` with `e.upstream_jsonrpc_code().unwrap_or(-32603)`. Frontend retry/decision logic that keys off codes (e.g. `-32601 Method not found` → don't retry, `-1 User rejected` → surface to user) now sees the real upstream code.

### Phase 4 — LOW audit fixes (2026-04-26)

A pass through the LOW severity items in `AUDIT_REPORT.md`. Most are local
correctness / hygiene / DX improvements that don't change wire shape or public
API. Highlights:

- **`turbomcp-core`: `ErrorKind::UrlElicitationRequired` variant added** — `crates/turbomcp-core/src/error.rs`. Pre-3.2.0 the `URL_ELICITATION_REQUIRED` (-32042) constant existed in `error_codes` but `ErrorKind::from_i32(-32042)` collapsed into `CapabilityNotSupported`, so callers couldn't distinguish the URL-elicitation case. Now it has a dedicated variant; `jsonrpc_error_code` and `http_status` (403) cover it. `JsonRpcIncoming.jsonrpc` field documented and given an `is_valid_version()` helper for callers that want to enforce the spec-required `"2.0"` value (raw deserialize stays lenient so the parse error path can still surface a useful `-32600 Invalid Request` for `1.0`/missing).
- **`turbomcp-types`: `ProtocolVersion::is_any_draft()` added** — `crates/turbomcp-types/src/primitives.rs`. The existing `is_draft()` only matched the named `Self::Draft` enum variant, so future drafts routed to `Unknown(_)` would return `false` and surprise callers gating on draft-only behaviour. New helper covers both. `validate_uri_template` now also checks RFC 6570 expression-body shape (rejects `{}`, `{1abc}`, `{ }`, `{foo bar}`) instead of only checking brace balance — `crates/turbomcp-types/src/definitions.rs`.
- **`turbomcp-transport-traits`**: `TransportMessageMetadata::with_ttl` now saturates at `u64::MAX` instead of silently truncating `u128` → `u64`. `TransportMessage::is_compressed` no longer reports `true` for `encoding: "identity"` (the HTTP-style "no compression" tag) — it now matches against a known compression set (`gzip`/`br`/`brotli`/`deflate`/`zstd`/`lz4`). `TransportEventEmitter` gains a `dropped_events()` counter so backpressure-induced loss is observable; pre-3.2.0 events overflowing the 500-slot channel were silently swallowed by `try_send` + `let _ =`. `crates/turbomcp-transport-traits/src/{events,message}.rs`.
- **`turbomcp-protocol::jsonrpc`: `JsonRpcError` predicates filled out** — `crates/turbomcp-protocol/src/jsonrpc.rs`. Added `is_method_not_found`, `is_invalid_params`, `is_internal_error`, `is_server_error`, and `standard_kind() -> Option<JsonRpcErrorCode>` so callers can stop matching on raw `e.code() == -32601` magic numbers. `validate_string_format("uri", …)` now requires an absolute URI per JSON Schema (uses `url::Url::parse`); pre-3.2.0 it accepted bare paths starting with `/`, so `/etc/passwd` passed validation as if it were a `Resource.uri`.
- **`turbomcp-protocol::state`: `StateManager` switched to `parking_lot::RwLock`** — `crates/turbomcp-protocol/src/state.rs`. Pre-3.2.0 every getter/setter was guarded by `if let Ok(state) = self.state.write()`, so a panic in any writer would silently disable `set` / `clear` / `import` for every subsequent call (poisoned `std::sync::RwLock`). `parking_lot::RwLock` has no poisoning, so a panic in one section now leaves the lock recoverable.
- **`turbomcp-core::router`**: `parse_request_from_value` added so the line-based transports (STDIO/TCP/Unix) can parse the incoming JSON once, branch on response-vs-request, and convert into `JsonRpcIncoming` without a second full parse. `crates/turbomcp-server/src/transport/line.rs` now uses the helper. Saves one `serde_json::from_str` per inbound message on the hottest server path.
- **`turbomcp-server`**: `ConnectionCounter::try_acquire_arc` no longer caps the CAS retry loop at 1000 iterations (the cap produced false-alarm `tracing::error!` logs under genuine `accept()` storms; unbounded CAS guarantees forward progress). `CompositeHandler::match_prefix` no longer allocates a `format!("{prefix}{sep}")` on every call — direct byte comparison, drop-in faster. `crates/turbomcp-server/src/{config,composite}.rs`.
- **`turbomcp-client`**: experimental Tasks methods (`get_task`, `cancel_task`, `list_tasks`, `get_task_result`) now gate on `initialized` like every other operation. `connect_tcp` rustdoc clarifies that DNS hostnames are NOT resolved (numeric `SocketAddr` only). `sampling::ServerInfo` renamed to `LlmServerInfo` (`ServerInfo` kept as deprecated alias) so it stops shadowing the MCP `Implementation`/`InitializeResult.server_info` shape at the prelude level. Emoji-prefixed log lines stripped from `core.rs` for structured-log consumers. `crates/turbomcp-client/src/{client/core,prelude,sampling}.rs`.
- **`turbomcp-http`**: `RetryPolicy::Exponential` jitter is now sourced from `fastrand` per-instance instead of a deterministic hash of the attempt counter. Pre-3.2.0 every client computing a retry on attempt N produced the same delay, defeating the thundering-herd defence the jitter exists to provide. New `fastrand` dep on `turbomcp-http`. `crates/turbomcp-http/src/transport.rs:88-106`.
- **`turbomcp-websocket`**: `bidirectional::stop_correlation` for non-existent correlations is now `debug!` instead of `warn!` — the race against timeout cleanup is normal and routinely produced spurious warnings. `crates/turbomcp-websocket/src/bidirectional.rs:140-145`.
- **`turbomcp-grpc`**: `convert::encode_json_map`'s `serde_json::to_vec(&value).expect(...)` replaced with `unwrap_or_else` + `tracing::warn!` + `b"null"` fallback so the hot conversion path can't panic even if a future `serde_json` change introduces a failure mode.
- **`turbomcp-auth`**: `DcrClient` now has a manual `Debug` impl that redacts `initial_access_token` (the IAT is a bearer credential — the previous derived `Debug` printed it in plaintext via `{:?}`). `crates/turbomcp-auth/src/oauth2/dcr.rs`.
- **`turbomcp-telemetry`**: `TelemetryGuard::drop` now mirrors tracer-provider shutdown errors to `eprintln!` in addition to `tracing::error!` — `Drop` may run after the tracing subscriber is deinitialized, in which case the structured log vanishes. `crates/turbomcp-telemetry/src/init.rs:178-193`.
- **`turbomcp-cli`**: `dev::is_command_available` switched from shelling out to `which` (Unix-only) to the cross-platform `which` crate. New dep, `which = "8.0"`.
- **`turbomcp-openapi`**: `lookup_ref` now follows reference chains up to 10 levels deep with cycle detection (pre-3.2.0 only one level of indirection was followed; `Foo → Bar → Baz` chains broke). `crates/turbomcp-openapi/src/provider.rs:582-606`.
- **`turbomcp-wasm`**: `OAuthProvider`'s authorization-server metadata no longer advertises `jwks_uri` until `handle_jwks` actually publishes keys (pre-3.2.0 the metadata pointed at an endpoint that returned an empty key set, misleading clients). `JwtConfig::leeway_seconds` semantics documented inline so the asymmetric `now > exp + leeway` vs `now + leeway < nbf` form is no longer a "what does this mean" moment for future maintainers. lib.rs Cargo example updated to match the actual workspace pins (`worker = "0.8"`, `getrandom = "0.4"`, `schemars = "1.2"`). `crates/turbomcp-wasm/src/{auth/{jwt,provider/mod},lib}.rs`. `turbomcp-wasm-macros` README updated to match.
- **`turbomcp-proxy`**: `MCP_PROTOCOL_VERSION` now re-exports `turbomcp_protocol::PROTOCOL_VERSION` (pre-3.2.0 it was a duplicated string constant that could drift). `BackendConnector::new`'s tautological `"127.0.0.1:0".parse().unwrap_or_else(|_| "127.0.0.1:0".parse().unwrap())` replaced with `SocketAddr::from(([127, 0, 0, 1], 0))`. `crates/turbomcp-proxy/src/{lib,proxy/backend}.rs`.

### Phase 3 — MED audit fixes (2026-04-26)

A pass through the MED severity items in `AUDIT_REPORT.md`. Most are local
correctness or hygiene improvements; a few are spec-conformance fixes (additive
schema fields, batch-deprecation handling). Highlights:

- **`turbomcp-core`: `from_rpc_code` now delegates to `ErrorKind::from_i32`** so the two canonicalization paths agree on `-32042` (URL elicitation required); `ErrorKind::Serialization` now maps to `-32603` (Internal) on the wire instead of colliding with `InvalidParams`. The `Json<T>` JSON-encoding logic in `IntoToolResponse`/`IntoToolResult` is deduplicated into `encode_json_for_tool`. `crates/turbomcp-core/src/{error,response}.rs`.
- **`turbomcp-types`: `_meta` typed consistently** across `wire.rs` types (`InitializeRequest`, `InitializeResult`, `CallToolResult`, `GetPromptResult`) — switched from `Option<Value>` to `Option<HashMap<String, Value>>` matching the rest of the crate and rejecting non-object values per spec. Field renamed `_meta → meta` with `#[serde(rename = "_meta")]` to clear the `clippy::pub_underscore_fields` lint. `PrimitiveSchemaDefinition::String/Number/Integer` gain the spec-required `default` field (MCP 2025-11-25 schema.ts:2250-2280). Inert `schema = ["dep:schemars"]` feature and the unused `schemars` workspace dep removed; `alloc` and `experimental-tasks` features documented as currently-decorative. `crates/turbomcp-types/{Cargo.toml,src/wire.rs,src/protocol_schemas.rs}`.
- **`turbomcp-transport-streamable`: `OriginValidation::validate_strict` added** for fail-closed origin validation (the existing `validate` is documented permissive). `Session::new` / `Session::touch` / `StoredEvent::new` no longer silently log `unwrap_or(0)` on `SystemTime` failure — a stderr warning is emitted via a new `now_millis_warn()` helper so the operator can spot the clock-before-epoch state instead of treating the session as instantly expired. `From<String>`/`From<&str>` for `SessionId` now have rustdoc flagging the panic-on-spec-violation behavior so callers handling untrusted input (`Mcp-Session-Id` header) reach for `try_from_string` instead.
- **`turbomcp-protocol::jsonrpc`: `JsonRpcError` size cap** — error `message` (and string-typed `data` fields) are bounded at 1 KiB with a `…[truncated, N bytes elided]` suffix so naive `JsonRpcError::invalid_params(&raw_input)` calls cannot amplify oversized client payloads back over the transport. New `parse_message_typed` helper distinguishes top-level batch arrays (`-32600 Invalid Request` with stable reason `"JSON-RPC batches are not supported in MCP 2025-11-25"`) from generic parse errors so callers can produce spec-recommended responses instead of echoing serde's diagnostic.
- **`turbomcp-protocol::types`: `LoggingMessageNotification` alias** added (spec name; `LoggingNotification` retained for backwards compat). `TaskStatusNotification` gains `lastUpdatedAt: Option<String>` per `schema.ts:1490` (`Task` flatten); pre-3.2.0 the field was silently dropped on deserialize.
- **`turbomcp-protocol::validation`: email and date format validation hardened.** Email validation now requires a domain with at least two labels with no empty segments (rejects `a@b.`, `@.`, `.@.`). Date validation uses `chrono::NaiveDate::parse_from_str` so `9999-13-99` is rejected on month/day range (chrono is lenient about width — single-digit months/days like `2025-1-7` still parse).
- **`turbomcp-protocol::session`: `SessionManager::start()` is idempotent.** Pre-3.2.0 the `cleanup_timer` field guard only protected the field assignment, not the `tokio::spawn`, so a second `start()` would leak a parallel cleanup loop on the same `DashMap`. Now an early return on `cleanup_timer.is_some()`.
- **`turbomcp-protocol::security::validation`: split syntactic vs filesystem path validation.** `validate_path_syntactic` runs only the textual checks (null bytes, URL-encoded traversal patterns, Unicode lookalikes); `validate_path` chains it before `canonicalize`. Previously callers wanting "validate before write" had to hand-roll the syntactic half. `decode_url_encoded` now loops to a fixed point (capped at 8 passes) instead of being hard-capped at 2, so triple-and-deeper-encoded payloads (`%25252e`) no longer slip past traversal detection.
- **TCP / Unix transports: non-UTF-8 payloads rejected** — `crates/turbomcp-tcp/src/transport.rs:528-538`, `crates/turbomcp-unix/src/transport.rs:533-545`. Pre-3.2.0 outbound payloads were `String::from_utf8_lossy`'d into the wire, which mangled unexpected bytes into U+FFFD and silently corrupted the JSON-RPC frame. Now `std::str::from_utf8` propagates `SerializationFailed` so the bug surfaces at the call site.
- **Axum subtree: JWT introspection logs SHA-256 prefix instead of token prefix** — `crates/turbomcp-transport/src/axum/middleware/auth.rs`. `tracing::warn!` now reports the first 4 bytes of the SHA-256 digest of the token rather than the literal first 8 chars; structured-prefix bearers (`sk-…`, version markers) no longer leak useful entropy to log sinks. New `sha2` dep on `turbomcp-transport`.
- **Axum subtree: explicit `-32600 Invalid Request` for batch JSON-RPC** — `crates/turbomcp-transport/src/axum/handlers/json_rpc.rs`. Handler now accepts `Json<serde_json::Value>` and returns the spec-stable batch-deprecation reason; previously a top-level array failed at axum's deserializer with a generic 400.
- **Client: `is_initialized()` accessor added** — `crates/turbomcp-client/src/client/core.rs`. Callers no longer need to pattern-match on `Error::invalid_request("Client not initialized")` to detect state. `AutoApprovingUserHandler::new()` now emits a `tracing::warn!` at construction so its development-only nature is auditable in deployed logs. `send_response` log levels demoted: full payload only at `trace!` (with explicit "may contain sensitive data" annotation), size+id at `debug!`, success at `debug!`. Removed emoji prefixes for structured-log consumers.
- **Server: clientInfo validated** — `crates/turbomcp-server/src/router.rs:96-130`. `clientInfo.name` and `clientInfo.version` now reject empty/whitespace-only/control-character/over-128-char values to defang log injection / telemetry noise via `tracing` attributes. `validate_protocol_header` now rejects post-init requests missing the `Mcp-Protocol-Version` header (was lenient pre-3.2.0); pre-init requests still allow absence so the very first POST `initialize` doesn't have to negotiate a version it hasn't seen yet.
- **Telemetry: production-safe defaults** — `crates/turbomcp-telemetry/src/config.rs`. `log_level` default flipped from `"info,turbomcp=debug"` to `"info"`; `RUST_LOG` still overrides via `EnvFilter::try_from_default_env()`. Pre-3.2.0 `TelemetryConfig::default().init()` silently turned on DEBUG logs across every workspace crate in production.
- **SDK: prelude `ClientCapabilities` collision resolved** — `crates/turbomcp/src/lib.rs`. The `client-integration` re-export aliases `turbomcp_client::ClientCapabilities` to `ClientCapsConfig` so it doesn't shadow `turbomcp_protocol::ClientCapabilities` at the crate root.
- **CLI: `create_stdio_transport` honors shell quoting** — `crates/turbomcp-cli/src/transport.rs`. Replaced `split_whitespace` with `shell_words::split`. Paths with spaces, `bash -c "…"` wrappers, and other shell-aware command shapes now parse correctly. New `shell-words` dep.
- **OpenAPI**: parser falls back to YAML when JSON parsing fails on a `{`-prefixed input (catches flow-style YAML documents); `reqwest::Client::builder().build()` failures are now `expect`'d instead of silently downgrading to `Client::new()` (which would have lost the user's configured timeout); `ExtractedOperation` gains `response_schema` populated from the first 2xx `application/json` response (with `$ref`s inlined), surfaced as MCP `Tool::output_schema`. `crates/turbomcp-openapi/src/{parser,provider,handler}.rs`.
- **WASM**: `is_valid_content_type` now requires a `Content-Type` header on POST (silently accepting missing `Content-Type` contradicted the file-level docstring). `MemorySessionStore`/`MemoryStore` `store_event` for unknown sessions now logs a warning so the silent no-op is observable. `crates/turbomcp-wasm/src/wasm_server/{handler,streamable}.rs`.

### Security

- **HTTP transport: cross-origin redirect with bearer token closed** — `crates/turbomcp-http/src/transport.rs:255-280`. When `auth_token` is configured, the reqwest client is now built with a custom redirect policy that follows redirects only to the same `Origin` as the previous request. Pre-3.2.0, reqwest's default `limited(10)` policy preserved the `Authorization: Bearer …` header across cross-origin redirects, leaking the token to whatever third-party host the server pointed at. Behaviour without an auth token is unchanged.
- **HTTP transport: SSE buffer cap added** — `crates/turbomcp-http/src/transport.rs:573-642, 947-1004`. Both the GET-SSE task and the POST→SSE response loop now drop the connection when the per-event accumulator exceeds `LimitsConfig.max_response_size` (default 10 MiB). Pre-3.2.0 a buggy or malicious server could stream gigabytes without ever emitting `\n\n` and OOM the client. Cap is gated on `LimitsConfig.enforce_on_streams` (default `true`) and `max_response_size: Some(_)`.
- **WebSocket transport: configured `max_message_size` enforced on the wire** — `crates/turbomcp-websocket/src/connection.rs:67-89`, `crates/turbomcp-websocket/src/tasks.rs:493-525`. Both initial connect and reconnect paths now call `connect_async_with_config` with a `WebSocketConfig` whose `max_message_size`/`max_frame_size` mirror the user's config (default 16 MiB). Pre-3.2.0 these calls used `connect_async(url)`, which silently inherited tungstenite's ~64 MiB default — the user's configured cap was only checked by the optional `validate_message` helper that is bypassed by `Transport::send` and never used on incoming frames.
- **WebSocket transport: phantom `enable_compression` and `tls_config` fields deprecated** — `crates/turbomcp-websocket/src/{config,types}.rs`. The transport now hard-advertises `supports_compression: false` (tungstenite 0.29 does not implement RFC 7692 permessage-deflate). The `enable_compression` field, the `with_compression` builder, the `tls_config` field, and the `with_tls_config` builder are marked `#[deprecated(since = "3.2.0")]` with notes pointing at their no-op nature. Pre-3.2.0 the capability advertisement claimed `supports_compression: true` and `compression_algorithms: ["deflate", "gzip"]` whenever the user asked for it — no compression actually happened on the wire, so peers were misled. TLS still works for `wss://` URLs via tokio-tungstenite's default rustls connector; the per-cert fields were never consulted.
- **TCP / Unix transports: server-mode broadcast warning** — `crates/turbomcp-tcp/src/transport.rs:531-562`, `crates/turbomcp-unix/src/transport.rs:534-571`. `Transport::send` still fans out to every connected peer in server mode (per-connection routing is a future-work item), but now logs a `tracing::warn!` whenever it broadcasts to >1 connection and the doc-comment explicitly documents the limitation. Multi-tenant deployments should not use these transports' server modes today; the warning makes the gap visible instead of silent.
- **Compression: decompression bombs bounded** — `crates/turbomcp-transport/src/compression.rs`. `MessageCompressor::decompress` now refuses to allocate past `max_decompressed_size` (default 16 MiB; override with `with_max_decompressed_size`). gzip / brotli reads are wrapped in `Read::take(cap+1)` and the result length is checked, so a high-ratio bomb (1 MiB → many GiB) returns `TransportError::ResponseTooLarge` instead of OOMing. LZ4 frames whose attacker-controlled size prefix exceeds the cap are rejected before any allocation.
- **Bidirectional correlation: requests now match on JSON-RPC `id`** — `crates/turbomcp-transport/src/bidirectional.rs`. `send_request`/`send_server_request` previously inserted a fresh-`Uuid` correlation key locally while sending the caller's payload unchanged on the wire — and the response path looked for a top-level `correlation_id` field that JSON-RPC peers never produce. Net effect: every request would time out instead of matching, even on a healthy connection. Now the correlation key is the JSON-RPC `id` extracted from the outgoing payload (rendered as a string so `42` and `"42"` share a namespace), and `extract_correlation_id` reads the JSON-RPC `id` from inbound responses. The legacy `correlation_id` field is no longer honoured.
- **Bidirectional correlation map: capped + auto-reaped** — `crates/turbomcp-transport/src/bidirectional.rs`. The `DashMap<String, CorrelationContext>` had no size limit and `start_correlation` entries were only removed by an explicit `stop_correlation`, so dropped requests leaked. New defaults: `DEFAULT_MAX_CORRELATIONS = 1024` (insert paths return `TransportError::RateLimitExceeded` past the cap; override via `with_max_correlations`) and a background reaper task that sweeps expired entries (`is_expired()` true) every 5 s, exiting cleanly when the wrapper is dropped.
- **`ServerTransportManager`: no-op dispatch replaced with real fan-out** — `crates/turbomcp-transport/src/server.rs`. `broadcast_to_all`, `send_server_request`, and `send_to_client` previously logged a warning, returned `NotAvailable` (or silent `Ok(())`) and threw away `TransportMessage` arguments — even though `supports_server_requests()` returned `true`. The "requires mutable access" justification was wrong: `Transport::send` and `BidirectionalTransport::send_request` both take `&self`. Implementations now actually dispatch through the underlying transport (snapshotting `Arc`s under the read lock and releasing it before awaiting the send so a long round-trip doesn't block `add_client`/`remove_client`).
- **HTTP server: `X-Forwarded-For` only honoured behind trusted proxies** — `crates/turbomcp-server/src/transport/http.rs:540-575` + `crates/turbomcp-server/src/config.rs`. `OriginValidationConfig` gains a `trusted_proxies: Vec<String>` allowlist (CIDR like `10.0.0.0/8` or bare addresses). When the immediate peer's IP is not in the allowlist, `extract_request_ip` now returns the peer IP verbatim and ignores `X-Forwarded-For`, `X-Real-IP`, `CF-Connecting-IP`, and `X-Client-IP`. Pre-3.2.0 a direct client could spoof any of those headers to (a) evade per-IP rate limiting, (b) satisfy origin validation's `client_ip.is_loopback()` short-circuit, or (c) influence session IP-binding decisions. The unsafe `extract_client_ip` helper is preserved for callers behind a trusted reverse proxy but its rustdoc now flags it as unsafe; new code should call `extract_client_ip_with_trust(headers, peer_ip, &trusted_proxies)`.
- **Origin validation: `starts_with` bypass closed; allowlist canonicalized** — `crates/turbomcp-transport/src/security/origin.rs`, `security/utils.rs`. `validate_origin` previously matched the localhost allowlist with `origin.starts_with("http://localhost")`, accepting `http://localhost.evil.com`, `http://localhost@evil.com`, `http://localhost:8080@evil.com`, and similar smuggle forms. The allowlist itself was a raw-string `HashSet` so `https://Example.com` ≠ `https://example.com` and `https://example.com` ≠ `https://example.com:443`. Both paths now canonicalize via `url::Url::parse` into a `(scheme, host, port_or_default)` triple, with userinfo / path / query / fragment rejected outright. `is_localhost_origin` in `security/utils.rs` was rewritten on the same parser. New regression tests cover the localhost-lookalike, userinfo, default-port, and path-suffix bypass shapes.
- **Telemetry: span error messages now truncated; high-cardinality attrs opt-out** — `crates/turbomcp-telemetry/src/tower/{mod,service}.rs`. `mcp.error.message` is now bounded by `TelemetryLayerConfig.error_message_max_len` (default 512 bytes; `0` drops the field entirely) before being recorded on a span. JSON-RPC error strings routinely contain echoed user input, file paths, SQL fragments, and backend stack traces — pre-3.2.0 the layer copied them verbatim into spans exported to the OTel collector, an exfiltration path. Two new toggles, `redact_request_id` and `redact_resource_uri`, suppress the per-request and client-controlled fields for cardinality-sensitive backends. Attribute cost is documented in the layer's rustdoc.
- **Telemetry: Prometheus listener defaults to `127.0.0.1`** — `crates/turbomcp-telemetry/src/{config,init}.rs`. New `prometheus_bind_addr: Option<IpAddr>` config field; when unset the recorder binds to loopback, and binding to a non-loopback address now emits a `WARN` log (raw runtime metrics are exposed without auth). Pre-3.2.0 the recorder hardcoded `0.0.0.0:{port}`, exposing tool/tenant/error labels on every interface to anyone who could reach the host.
- **Telemetry: `OtlpProtocol` default flipped to `Http` to match what's built in** — `crates/turbomcp-telemetry/src/{config,init}.rs`. The crate ships only the HTTP/protobuf exporter (`opentelemetry-otlp` features `http-proto, reqwest-blocking-client`); selecting `OtlpProtocol::Grpc` was silently ignored and HTTP was used regardless. `OtlpProtocol::default()` is now `Http`, and explicitly choosing `Grpc` logs a clear warning at init pointing at the missing `grpc-tonic` exporter so users notice instead of getting silent collector rejections.
- **OpenAPI: `securitySchemes` and per-operation `security` are now propagated** — `crates/turbomcp-openapi/src/{provider,handler,lib}.rs`. New `AuthProvider` trait lets callers inject credentials matching each operation's `SecurityRequirement`s; new `OpenApiProvider::with_auth_provider(...)` builder method installs it. Operation-level `security` overrides spec-level (an explicit empty list disables auth). Each `ExtractedOperation` now carries its effective requirements; tools/resources surface them in `meta["security"]` plus `meta["securitySchemes"]` so MCP clients can detect auth requirements without re-fetching the spec. Pre-3.2.0 the converted MCP tool had no way to declare auth requirements and any tool call against an authenticated upstream returned 401 unless the user manually injected headers via `with_client`.
- **OpenAPI: `servers:` now seeds `base_url`** — `crates/turbomcp-openapi/src/provider.rs`. `OpenApiProvider::from_spec` reads `spec.servers[0].url` (with default `variables` substituted) and parses it into `base_url`. Pre-3.2.0 the field was always `None`, so any tool call without a prior `with_base_url(...)` returned `OpenApiError::NoBaseUrl`. `with_base_url` continues to override.
- **WASM: OAuth token store is now mandatory at construction** — `crates/turbomcp-wasm/src/auth/provider/mod.rs`. `OAuthProvider::new(config, store)` requires a `SharedTokenStore` parameter so durable-storage decisions are explicit at the call site; the named `OAuthProvider::with_memory_store(config)` exists for tests/local dev so the trade-off carries into code review. Pre-3.2.0 the default constructor silently installed `MemoryTokenStore`, and a Cloudflare Workers deploy that forgot `with_store(...)` would lose every token on isolate restart (~15-30 minutes).
- **Proxy: backend bearer token wrapped in `SecretString` past the configuration boundary** — `crates/turbomcp-proxy/src/proxy/backend.rs`, `runtime/mod.rs`. `BackendTransport::Http.auth_token` is now `Option<SecretString>` (redacted in `Debug`, zeroed on drop). `BackendConnector::new`'s `info!("Creating backend connector: {:?}", config.transport)` was the audit's concrete leak path: the bearer was printed verbatim at INFO level. The log now emits only the transport-kind discriminant via a new `backend_transport_kind` helper. The outer `BackendConfig::Http.auth_token` (in `crates/turbomcp-proxy/src/config.rs`) stays `Option<String>` because the enum is `Serialize`-deriving for config-file round-trip and `secrecy` requires `SerializableSecret` opt-in; the secret is wrapped immediately when it crosses into the runtime layer.
- **Proxy: HTTP/WebSocket frontends now enforce Origin allowlist + CORS** — `crates/turbomcp-proxy/src/runtime/security.rs` (new module), `runtime/mod.rs`, `cli/commands/serve.rs`. New `OriginAllowlist` shared state + `origin_guard` axum middleware reject any request carrying an `Origin` header that is not on the allowlist (literal `null` always rejected). Empty allowlist (the default) means *no browser-issued requests are accepted* on either frontend — server-to-server clients without `Origin` are unaffected. When the allowlist is non-empty a `CorsLayer` is also installed advertising `mcp-session-id` and `last-event-id` per Streamable HTTP. Configured via `RuntimeProxyBuilder::with_allowed_origins([...])` and the new `--allowed-origin` CLI flag (repeatable). Pre-3.2.0 the proxy's primary ingress had neither check, so a malicious page in any browser could target a localhost-bound proxy carrying full session capability.

### MCP 2025-11-25 spec compliance

- **Server: missing spec methods `resources/subscribe`, `resources/unsubscribe`, `logging/setLevel`, `completion/complete` are now routed** — `crates/turbomcp-core/src/handler.rs`, `crates/turbomcp-core/src/router.rs`. Pre-3.2.0 the core router only handled `initialize` / `tools/*` / `resources/{list,read}` / `prompts/*` / `tasks/*` / `ping`, so the four spec methods above fell through to a `-32601 Method not found` even when the server advertised the matching capability. The `McpHandler` trait now has default-impl methods (`subscribe`, `unsubscribe`, `set_log_level`, `complete`) that return `capability_not_supported`; the router wires the four methods through. Servers that opt into `resources.subscribe` or the `logging` capability MUST override; non-overriding servers correctly report capability-not-supported instead of method-not-found. The `complete` signature uses raw `serde_json::Value` (`CompleteRequestParams`/`CompleteResult` live in `turbomcp-protocol` which depends on `turbomcp-core`, so we cannot import the typed shapes here without inverting the layer cake — typed wrappers can be exposed at higher levels).
- **Server: `notifications/cancelled` now actually cancels in-flight handlers** — `crates/turbomcp-server/src/transport/{line,channel,websocket}.rs`. Pre-3.2.0 the cancellation notification was forwarded to `route_request`, which produced a method-not-found response that was silently dropped (notifications have no id). The transport runners now maintain a per-connection `DashMap<request_id, CancellationToken>`: each non-init request gets a fresh token installed into its `RequestContext` and registered in the map; the spawned handler task removes the entry on completion; receipt of `notifications/cancelled` parses the `requestId`, looks up the token, and calls `.cancel()`. Handlers that periodically check `ctx.is_cancelled()` (or that consume cancellable Futures) now stop runaway work as soon as the client signals. The websocket transport additionally got a structural fix: it now spawns each handler on its own task so the receive loop can actually deliver the cancellation notification while a handler is mid-flight (pre-3.2.0 the loop awaited each handler synchronously, which would have prevented cancellation even with the registry).
- **Client: outgoing `notifications/cancelled` and cancellation-safe waiter cleanup** — `crates/turbomcp-client/src/client/{dispatcher,protocol,operations/connection}.rs`. New public `Client::cancel_request(&id, reason)` API (per MCP §Cancellation: callers SHOULD signal abandonment so the server can stop work). The internal request path now also sends `notifications/cancelled` automatically when the per-request timeout fires. Separately, `MessageDispatcher::wait_for_response_guarded` returns an RAII `WaiterGuard` whose `Drop` removes the entry from `response_waiters` if the awaiting future is dropped mid-flight (`tokio::select!`, `tokio::time::timeout`, structured-concurrency abort). Pre-3.2.0 long-lived clients with many cancelled requests slowly accumulated `oneshot::Sender`s in the dispatcher map until shutdown.
- **`ProgressNotification` aligned to `progressToken: string | number` and `progress/total: number`** — `crates/turbomcp-protocol/src/types/logging.rs:71-90`. Per `schema.ts:23,1551-1561` the token is a string-or-number (typically mirrored from `_meta.progressToken` or the request's numeric ID) and progress values are JSON numbers (floats). Pre-3.2.0 these were `String`/`u64`/`Option<u64>`, so receiving a numeric token from a spec-compliant peer failed deserialization and fractional progress was lost. Added `pub type ProgressToken = MessageId` (`types/core.rs`) and updated `RichContextExt::report_progress*` signatures (`context/rich.rs:261-279, 392-426`) to take `f64` / `Option<f64>` and `impl Into<ProgressToken>`. **Public API change**: callers of `report_progress(u64, u64, …)` and `report_progress_with_token(impl Into<String>, u64, Option<u64>, …)` need to convert numeric arguments to `f64` (`50` → `50.0`).
- **`ClientRequest` enum gains the Tasks API (SEP-1686)** — `crates/turbomcp-protocol/src/types/requests.rs:24-95`. `tasks/get`, `tasks/result`, `tasks/list`, `tasks/cancel` are client-initiated per `schema.ts:2520-2527`. New variants: `ClientRequest::TasksGet/TasksResult/TasksList/TasksCancel`. The corresponding param types already lived in `types::tasks`; this just wires the typed routing enum so callers no longer fall back to ad-hoc `serde_json::Value`.
- **`ServerNotification` and `ClientNotification` enums updated for bidirectional notifications** — `crates/turbomcp-protocol/src/types/requests.rs:103-159`. Per `schema.ts:2535,2562-2570`:
  - `ServerNotification` adds `ElicitationComplete` (`notifications/elicitation/complete`) and `TaskStatus` (`notifications/tasks/status`).
  - `ClientNotification` adds `Cancelled` (`notifications/cancelled`) and `TaskStatus` (`notifications/tasks/status`) — both bidirectional per spec.

### Deprecated

- **`turbomcp-transport::axum` subtree is deprecated** — `crates/turbomcp-transport/src/axum/`. The four documented public-API types — `AxumMcpExt`, `McpAppState`, `McpServerConfig`, `McpService` — are now `#[deprecated(since = "3.2.0")]` at their source definitions, so any path that resolves to them (whether `turbomcp_transport::AxumMcpExt` or `turbomcp_transport::axum::AxumMcpExt`) emits a migration warning. The subtree predates the MCP 2025-11-25 Streamable HTTP rework and lacks `Mcp-Session-Id` lifecycle, `Last-Event-ID` resumption, and the unified `/mcp` method-multiplexed endpoint that `turbomcp-server::transport::http` already implements (see `crates/turbomcp-server/src/transport/http.rs:503,750,761`). Keeping two HTTP server stacks is the dominant systemic theme in the v3.1.1 audit (multiple HIGHs against the axum stack — origin/session/SSE-broadcast/rate-limit gaps — that have no analog in the spec-compliant stack).

  **Migration**: serve over `turbomcp_server::transport::http` instead. For server crates, prefer the `#[server]` macro's transport selection (`Transport::http("0.0.0.0:8080")` requires the `http` feature). The deprecated subtree continues to compile until removal in a future major release; in-tree code (the `turbomcp-proxy` HTTP frontend) is silenced via `#![allow(deprecated)]` with a TODO to migrate alongside subtree removal.
