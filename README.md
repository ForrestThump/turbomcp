...
- **Transports behind one builder.** stdio (default) and Streamable HTTP (axum).
  `MyServer.run_stdio()` or `MyServer.into_server().run_http(addr, cfg)`.
...
- **Production seams.** OAuth 2.1 resource-server auth, identity-keyed rate
  ...
  bidirectional elicitation — each opt-in behind a feature flag.
...
| `turbomcp-codec` | Wire codec: bytes ↔ `JsonRpcMessage` (serde_json / sonic-rs / msgpack) |
...
TurboMCP interoperates with the official Rust SDK (rmcp) in both directions — a
TurboMCP client against an rmcp server and vice-versa — verified in-repo.
